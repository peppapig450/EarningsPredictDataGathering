...
## Project Wide Enum for API names
Implement an Enum that holds all API names used in the project, this would be useful for API keys,
and for filtering based on the API, or anything that is based on the API. This improves type checking.

## ClientSession manager
Create ClientSessions for each api, setting a base url, and the api keys for that API.
That way we can use that for all the requests to it and avoid having to inject api keys across objects.

## Task Creating
Rather than using a dequeue for the current_category, have each "symbols window" have flags for each data_category, so that once a result for one category is processed, its immediately put back into the io_queue

- this can probably be done with a callback signaling the TaskHandler class to change the datacategory, and the Tasktype and put it back in the appropiate queue.

## Symbols Dispatching
To handle either the symbols as strings or as the dataclasses, use a generic class such as this 
```python
from typing import Union, Generic

T = TypeVar('T', EarningsReport, str)  # Generic type variable for either report or string

class Symbol(Generic[T]):
    """Generic class representing either a full earnings report or just a symbol string."""

    def __init__(self, data: T):
        self.data: T = data

    def get_symbol(self) -> str:
        """Extracts the symbol from the data, regardless of its type."""
        if isinstance(self.data, EarningsReport):
...
Use a Session Handler that can take APIRequests (dataclass) and run them under an aiohttp pool, which task handler takes in a callback and handles further.
The functions to run the api request as well as the processing, will be contained in each subclass of Task, with it being run by the appropiate handdler.

## UVloop
- Use uvloop on macos for increased asyncio speed

## Global Sleeping and maximum efficiency
- sleeping could be achieved per data task by SessionHandler
